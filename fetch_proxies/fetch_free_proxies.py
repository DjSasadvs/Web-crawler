...
# This use to fetch the proxy in the Internet
# Cross the web-url:
# http://www.kxdaili.com, http://www.proxy.mimvp.com/free.php, http://www.ip181.com/
...
def fetch_all(endpage=2):
    ...
    for p in proxyes:
        if check(p):
            valid_proxyes.append("    {'ip_port': '%s', 'user_pass': ''},"%p)
...
if __name__ == '__main__':
    ...
    file = codecs.open('ip_proxies.json', 'w', encoding='utf-8')
    for p in proxyes:
        print p
        file.write(str(p)+"\n")
    file.close()
